...
public class FightMenu<T extends Enum>  implements Serializable{
    ...
    private final Runnable drawWithWarningMessage = (Runnable & Serializable)() -> {printMenuFooter(true);};

    private final IntRange acceptableItems;

    @SafeVarargs
    public FightMenu(String title, T... items) {
        if (items.length == 0) {
    ...
    private int readItemIndex() {
        return reader.readIntegerUntil(itemIsInAcceptableRange(), drawWithWarningMessage) - MENU_ITEM_OFFSET;
    }
